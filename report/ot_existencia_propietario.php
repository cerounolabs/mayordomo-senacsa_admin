<?php
    ...
    if ($workCodigo <> 0){
        ...
        $mpdf -> WriteHTML('<body>');
        $mpdf -> WriteHTML('<br>');
        $mpdf -> WriteHTML('<h2 style="margin:0px;">EXISTENCIA POR PROPIETARIO</h2>');
        $mpdf -> WriteHTML('<br>');
        ...
        if ($propietarioJSON['code'] == 200) {
            foreach ($propietarioJSON['data'] as $propietarioKey=>$propietarioArray) {
                                    ...
                                    if ($row_subcategoria_02 == 'ANIMALSUBCATEGORIA'){
                                        if ($otAudJSON['code'] == 200) {
                                            ...
                                            foreach ($otAudJSON['data'] as $ot_auditadaKey=>$ot_auditadaArray) {
                                                ...
                                                $row_auditada_08  = $ot_auditadaArray['propietario_codigo'];
                                                $row_auditada_09  = $ot_auditadaArray['propietario_marca'];

                                                if ($row_auditada_08 == $row_propietario_00 && $row_auditada_06 == $row_ot_07 && $row_auditada_02 == $row_categoria_00 && $row_auditada_04 == $row_subcategoria_00){
                                                    $detCategoria   = $row_auditada_03;
                                                    $detSubCategoria= $row_auditada_05;
                                                    $detCantidad    = $detCantidad + $row_auditada_07;
                                                    $cantPropietario= $cantPropietario + $row_auditada_07;
                                            ...
                                            if ($impDetalle == true){
                                                if ($impTitulo == true){
                                                    $mpdf -> WriteHTML('<tr><td style="text-align:left; font-weight:bold;" colspan="3">PROPIETARIO '.$row_propietario_01.'</td></tr>');
                ...
                if ($impCorte == true){
                    $cantPropietario     = number_format($cantPropietario, 0, ',', '.');
                    $mpdf -> WriteHTML('<tr> <td style="text-align:left; font-weight:bold;" colspan="2"> SUB-TOTAL PROPIETARIO '.$row_propietario_01.'</td><td style="text-align:right; font-weight:bold;">'.$cantPropietario.'</td></tr>');
        ...
        $mpdf -> Output('otExistenciaPorPropietario.pdf', 'I');
        exit;
    }
?>