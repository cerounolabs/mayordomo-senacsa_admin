<?php
    ...
    if ($workCodigo <> 0){
        ...
        $mpdf -> WriteHTML('<body>');
        $mpdf -> WriteHTML('<br>');
        $mpdf -> WriteHTML('<h2 style="margin:0px;">EXISTENCIA POR RAZA</h2>');
        $mpdf -> WriteHTML('<br>');
        ...
        if ($dominioJSON['code'] == 200) {
            foreach ($dominioJSON['data'] as $razaKey=>$razaArray) {
                $row_raza_00    = $razaArray['dominio_codigo'];
                $row_raza_01    = $razaArray['dominio_nombre'];
                $row_raza_02    = $razaArray['dominio_valor'];
                $impTitulo      = true;
                $impCorte       = false;
                ...
                if ($row_raza_02 == 'ANIMALRAZA'){
                                        ...
                                        if ($row_subcategoria_02 == 'ANIMALSUBCATEGORIA'){
                                            if ($otAudJSON['code'] == 200) {
                                                ...
                                                foreach ($otAudJSON['data'] as $ot_auditadaKey=>$ot_auditadaArray) {
                                                    ...
                                                    $row_auditada_08  = $ot_auditadaArray['raza_codigo'];
                                                    $row_auditada_09  = $ot_auditadaArray['raza_nombre'];

                                                    if ($row_auditada_08 == $row_raza_00 && $row_auditada_06 == $row_ot_07 && $row_auditada_02 == $row_categoria_00 && $row_auditada_04 == $row_subcategoria_00){
                                                        $detCategoria   = $row_auditada_03;
                                                        $detSubCategoria= $row_auditada_05;
                                                        $detCantidad    = $detCantidad + $row_auditada_07;
                                                        $cantRaza       = $cantRaza + $row_auditada_07;
                                                ...
                                                if ($impDetalle == true){
                                                    if ($impTitulo == true){
                                                        $mpdf -> WriteHTML('<tr><td style="text-align:left; font-weight:bold;" colspan="3">RAZA '.$row_raza_01.'</td></tr>');
                    ...
                    if ($impCorte == true){
                        $cantRaza       = number_format($cantRaza, 0, ',', '.');
                        $mpdf -> WriteHTML('<tr> <td style="text-align:left; font-weight:bold;" colspan="2"> SUB-TOTAL RAZA '.$row_raza_01.'</td><td style="text-align:right; font-weight:bold;">'.$cantRaza.'</td></tr>');
        ...
        $mpdf -> Output('otExistenciaPorRaza.pdf', 'I');
        exit;
    }
?>