<?php
    ...
    if ($workCodigo <> 0){
        ...
        $mpdf -> WriteHTML('<body>');
        $mpdf -> WriteHTML('<br>');
        $mpdf -> WriteHTML('<h2 style="margin:0px;">EXISTENCIA POR ORIGEN</h2>');
        $mpdf -> WriteHTML('<br>');
        ...
        if ($dominioJSON['code'] == 200) {
            foreach ($dominioJSON['data'] as $origenKey=>$origenArray) {
                $row_origen_00  = $origenArray['dominio_codigo'];
                $row_origen_01  = $origenArray['dominio_nombre'];
                $row_origen_02  = $origenArray['dominio_valor'];
                $impTitulo      = true;
                $impCorte       = false;

                if ($row_origen_02 == 'ANIMALORIGEN'){
                    if ($dominioJSON['code'] == 200) {
                        foreach ($dominioJSON['data'] as $categoriaKey=>$categoriaArray) {
                            ...
                            if ($row_categoria_02 == 'ANIMALCATEGORIA'){
                                if ($dominioJSON['code'] == 200) {
                                    foreach ($dominioJSON['data'] as $subcategoriaKey=>$subcategoriaArray) {
                                        ...
                                        if ($row_subcategoria_02 == 'ANIMALSUBCATEGORIA'){
                                            if ($otAudJSON['code'] == 200) {
                                                ...
                                                foreach ($otAudJSON['data'] as $ot_auditadaKey=>$ot_auditadaArray) {
                                                    ...
                                                    $row_auditada_08  = $ot_auditadaArray['origen_codigo'];
                                                    $row_auditada_09  = $ot_auditadaArray['origen_nombre'];

                                                    if ($row_auditada_08 == $row_origen_00 && $row_auditada_06 == $row_ot_07 && $row_auditada_02 == $row_categoria_00 && $row_auditada_04 == $row_subcategoria_00){
                                                        $detCategoria   = $row_auditada_03;
                                                        $detSubCategoria= $row_auditada_05;
                                                        $detCantidad    = $detCantidad + $row_auditada_07;
                                                        $cantOrigen     = $cantOrigen + $row_auditada_07;
                                                ...
                                                if ($impDetalle == true){
                                                    if ($impTitulo == true){
                                                        $mpdf -> WriteHTML('<tr><td style="text-align:left; font-weight:bold;" colspan="3">ORIGEN '.$row_origen_01.'</td></tr>');
                    ...
                    if ($impCorte == true){
                        $cantOrigen     = number_format($cantOrigen, 0, ',', '.');
                        $mpdf -> WriteHTML('<tr> <td style="text-align:left; font-weight:bold;" colspan="2"> SUB-TOTAL ORIGEN '.$row_origen_01.'</td><td style="text-align:right; font-weight:bold;">'.$cantOrigen.'</td></tr>');
        ...
        $mpdf -> Output('otExistenciaPorOrigen.pdf', 'I');
        exit;
    }
?>